...
const RAC_BUILDINGS = [
  ...
  {
    title: "145 King West",
    address: "145 King St W, Toronto",
    lat: 43.6484,
    lng: -79.384,
  },
  {
    title: "100 Adelaide West",
  ...
  useEffect(() => {
    ...
    function initMap() {
      ...
      for (const building of RAC_BUILDINGS) {
        ...
        marker.addListener("click", () => {
          infoWindow.setContent(
            `<div style="padding:4px 2px;font-family:sans-serif"><strong style="font-size:14px">${building.title}</strong><br/><span style="color:#555;font-size:12px">${building.address}</span></div>`,
          );
      ...
      new window.markerClusterer.MarkerClusterer({
        map,
        markers,
        algorithm: new window.markerClusterer.GridAlgorithm({ gridDistance: 60 }),
        renderer: {
    ...
    window.initGoogleMapsDemo = initMap;

    // If Maps API is already loaded, call init directly
    if (window.google?.maps && window.markerClusterer) {
      ...
      loadScript("https://unpkg.com/@googlemaps/markerclusterer/dist/index.min.js")
        .then(() =>
          loadScript(
            `https://maps.googleapis.com/maps/api/js?key=${apiKey}&callback=initGoogleMapsDemo`,
          ),
        )
        .catch(() => {
  ...
  if (status === "missing-key") {
    return (
      ...
      >
        ...
        to <code style={{ background: "var(--neutral-alpha-weak)", padding: "2px 6px", borderRadius: "4px" }}>.env.local</code> to display the live demo.
      </div>
    );
  }
